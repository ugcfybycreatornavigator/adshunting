...
export function dbAdToNormalized(row: DatabaseAdRow): NormalizedAd {
  ...
  return {
    ...
    intelligenceLabels: [intelligence.badgeCategory, ...(runningDays && runningDays >= 30 ? [`${runningDays}d Running`] : [])],
    rawData: row.raw_data ?? undefined,
  };
}

export function normalizedToDb(ad: NormalizedAd) {
  return {
    ...
    ...(ad.rawData ? { raw_data: ad.rawData } : {}),
  };
}
