  ...
  return <div className="flex h-full flex-col p-4 md:p-8 max-w-[1600px] mx-auto w-full">
    ...
    <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div className="relative w-full max-w-lg">
        ...
        <input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search saved creatives, notes, tags…" className="h-11 w-full rounded-lg border border-line pl-10 pr-3 text-sm outline-none focus:border-signal" />
      </div>
    ...
    {!filtered.length ? (
      <EmptyState icon={<BookmarkX />} title={items.length ? "No saved ads match" : "No saved creatives yet"} body={items.length ? "Try another search term." : "Save ads from Discover and they'll appear here."} action={!items.length ? <a href="/discover" className="inline-flex h-9 items-center justify-center rounded-md bg-signal px-4 py-2 text-sm font-medium text-white transition hover:opacity-90">Discover Ads</a> : undefined} />
    ) : (
...
function NotesModal({ item, onClose, onChange }: { item: Saved; onClose: ()=>void; onChange:(item:Saved)=>void }) {
  ...
  return <div className="fixed inset-0 z-[80] grid place-items-end bg-black/25 sm:place-items-center sm:p-4" onMouseDown={onClose}><div className="max-h-[92vh] w-full overflow-y-auto rounded-t-2xl bg-white p-5 shadow-2xl sm:max-w-lg sm:rounded-2xl" onMouseDown={e=>e.stopPropagation()}><div className="flex items-center justify-between"><div><h2 className="text-lg font-semibold">Creative notes</h2><p className="mt-1 text-xs text-muted">{item.ad.advertiserName}</p></div><button onClick={onClose} className="grid size-10 place-items-center"><X size={18}/></button></div><label className="mt-5 block text-xs font-semibold text-muted">Notes<textarea value={notes} onChange={e=>setNotes(e.target.value)} rows={5} maxLength={5000} placeholder="Capture the hook, angle, offer, or why this creative matters…" className="mt-2 w-full resize-none rounded-lg border border-line p-3 text-sm leading-6 outline-none focus:border-signal" /></label><Button onClick={saveNotes} className="mt-2" disabled={busy}>{busy?<Loader2 className="animate-spin" size={15}/>:<Check size={15}/>}Save notes</Button><div className="mt-6 border-t border-line pt-5"><p className="flex items-center gap-1.5 text-xs font-semibold text-muted"><Folder size={13}/>Swipe files</p><div className="mt-2 grid gap-2 sm:grid-cols-2">{collections.map(collection=><label key={collection.id} className="flex min-h-10 cursor-pointer items-center gap-2 rounded-lg border border-line px-3 text-xs font-medium"><input type="checkbox" className="accent-signal" checked={collectionIds.includes(collection.id)} onChange={()=>setCollectionIds(current=>current.includes(collection.id)?current.filter(id=>id!==collection.id):[...current,collection.id])}/><span className="truncate">{collection.name}</span></label>)}</div>{collections.length?<Button variant="secondary" className="mt-3" onClick={saveCollections} disabled={busy}>{busy?<Loader2 className="animate-spin" size={15}/>:<Check size={15}/>}Save folders</Button>:<p className="mt-2 text-xs text-muted">Create a swipe file first, then assign this creative to it.</p>}</div><div className="mt-6 border-t border-line pt-5"><p className="text-xs font-semibold text-muted">Tags</p><div className="mt-2 flex flex-wrap gap-2">{item.tags.map(current=><span key={current.id} className="flex items-center gap-1 rounded-full bg-red-50 px-2.5 py-1.5 text-xs font-medium text-signal"><Tag size={11}/>{current.name}<button onClick={()=>removeTag(current.id)}><X size={11}/></button></span>)}</div><form onSubmit={addTag} className="mt-3 flex gap-2"><input value={tag} onChange={e=>setTag(e.target.value)} placeholder="UGC, Hook, Social Proof…" className="h-10 min-w-0 flex-1 rounded-lg border border-line px-3 text-sm outline-none focus:border-signal"/><Button type="submit" variant="secondary" disabled={busy||!tag.trim()}>Add tag</Button></form></div>{error&&<p className="mt-3 rounded-lg bg-red-50 p-3 text-xs text-signal">{error}</p>}</div></div>;
}
